...
DOLS = [
    (
        DolHeader(
            sections=(
                Section(offset=256, base_address=2147496192, size=1344),
            ...
            bss_address=2151438880,
            bss_size=368356,
            entry_point=2147496588,
        ),
...
def test_apply_beam_cost_patch(dol_file):
    ...
    assert results == (
        b"\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x01\x00\x00\x00\x00"
...
def test_apply_safe_zone_heal_patch(dol_file):
    ...
    dol_file.set_editable(True)
    with dol_file:
        dol_patches.apply_safe_zone_heal_patch(addresses, sda2_base, 1.0, dol_file)

    # Assert
    results = dol_file.dol_path.read_bytes()[0x100:]
    expected = (
        b"\x3c\x88\x88\x89\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
