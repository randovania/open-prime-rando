...
import pytest_mock
from open_prime_rando.dol_patching import echoes_dol_patcher, echoes_dol_versions
...
def test_apply_patches(mocker: pytest_mock.MockerFixture):
    ...
    mock_find_version_for_dol = mocker.patch(
        ...
        return_value=version_patches, autospec=True,
    )
    dol_file = MagicMock()
    ...
    mock_apply_game_options: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_game_options_patch", autospec=True)
    ...
    mock_apply_beam_cost_patch: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_beam_cost_patch", autospec=True)
    mock_apply_starting_visor_patch: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_starting_visor_patch", autospec=True)
    mock_apply_fixes: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_fixes", autospec=True)
    mock_apply_unvisited_room_names: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_unvisited_room_names", autospec=True)
    mock_apply_teleporter_sounds: MagicMock = mocker.patch(
        "open_prime_rando.dol_patching.echoes_dol_patches.apply_teleporter_sounds", autospec=True)
